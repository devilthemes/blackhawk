...
<div id="cart_block" class="block exclusive">
	...
	{if $discounts|@count > 0}
		...
		{/if}
		<p id="cart-buttons">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process", true)}" class="button smallBtn" title="{l s='Cart' mod='blockcart'}">{l s='Cart' mod='blockcart'}</a>{/if}
			<a href="{$link->getPageLink("$order_process", true)}" id="button_order_cart" class="exclusive smallBtn button{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart'}"><span></span>{l s='Check out' mod='blockcart'}</a>
		</p>
	</div>
	</div>
</div>

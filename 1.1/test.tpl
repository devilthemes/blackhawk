...
{if isset($lastProductAdded) AND $lastProductAdded}
		...
		{if $product.id_product == $lastProductAdded.id_product AND (!$product.id_product_attribute OR ($product.id_product_attribute == $lastProductAdded.id_product_attribute))}
			{*<div class="cart_last_product">
				...
				<div class="cart_last_product_content">
					<h4><a href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'}</a></h4>
	...
	{/foreach}
{/if}
<p class="mycartinfo">{l s='Your shopping cart contains'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}</span></p>
<div id="order-detail-content" class="table_block">
	<div id="cart_summary" class="std">
		...
		<li id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" class="alternate_item cart_item ">
			...
			{if isset($customizedDatas.$productId.$productAttributeId)}
				{foreach from=$customizedDatas.$productId.$productAttributeId key='id_customization' item='customization'}
						...
						<div class="cart_quantity">
							<div >
								<a rel="nofollow" class="cart_quantity_delete" id="{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" href="{$link->getPageLink('cart.php', true)}?delete&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$token_cart}">{l s='Delete'}</a>
							</div>
		...
		<ul class="tfoot">
			{if $use_taxes}
				{if $priceDisplay}
					<li class="cart_total_price row">
						<div class="tencol"><div class="gap">{l s='Total products'}{if $display_tax_label} {l s='(tax excl.)'}{/if}{l s=':'}</div></div>
						<div class="price twocol last" id="total_product"><div class="gap">{displayPrice price=$total_products}</div></div>
					</li>
				{else}
					<li class="cart_total_price row">
						<div class="tencol"><div class="gap">{l s='Total products'}{if $display_tax_label} {l s='(tax incl.)'}{/if}{l s=':'}</div></div>
			...
			{/if}
			<li class="cart_total_voucher row" {if $total_discounts == 0}style="display: none;"{/if}>
				<div class="tencol">
					<div class="gap">
				{if $use_taxes}
					{if $priceDisplay}
				...
				<div class="price-discount twocol last" id="total_discount">
					<div class="gap">
				{if $use_taxes}
					{if $priceDisplay}
			...
			<li class="cart_total_voucher row" {if $total_wrapping == 0}style="display: none;"{/if}>
				<div class="tencol">
					<div class="gap">f
				{if $use_taxes}
					{if $priceDisplay}
				...
				<div class="price-discount twocol last" id="total_wrapping">
					<div class="gap">
				{if $use_taxes}
					{if $priceDisplay}
			...
			{if $use_taxes}
				{if $priceDisplay}
					<li class="row cart_total_delivery" {if $shippingCost <= 0} style="display:none;"{/if}>
						<div class="tencol"><div class="gap">{l s='Total shipping'}{if $display_tax_label} {l s='(tax excl.)'}{/if}{l s=':'}</div></div>
						<div class="price twocol last" id="total_shipping"><div class="gap">{displayPrice price=$shippingCostTaxExc}</div></div>
					</li>
				{else}
					<li class="row cart_total_delivery"{if $shippingCost <= 0} style="display:none;"{/if}>
						<div class="tencol" ><div class="gap">{l s='Total shipping'}{if $display_tax_label} {l s='(tax incl.)'}{/if}{l s=':'}</div></div>
						<div class="twocol price last" id="total_shipping" ><div class="gap">{displayPrice price=$shippingCost}</div></div>
					</li>
				{/if}
			{else}
				<li class="row cart_total_delivery"{if $shippingCost <= 0} style="display:none;"{/if}>
			...
			<li class="cart_total_price row">
				...
				<div class="price twocol last" id="total_price_without_tax"><div class="gap">{displayPrice price=$total_price_without_tax}</div></div>
			</li>
			<li class="cart_total_tax row">
				<div class="tencol">
					<div class="gap">
					{if $display_tax_label}
						{l s='Total tax:'}
				...
				<div class="price twocol last" id="total_tax"><div class="gap">{displayPrice price=$total_tax}</div></div>
			</li>
			<li class="cart_total_price row">
				<div class="tencol">
					<div class="gap">
				...
				<div class="price twocol last" id="total_price"><div class="gap">{displayPrice price=$total_price}</div></div>
			</li>
			{else}
			<li class="cart_total_price row">
				<div class="tencol"><div class="gap">{l s='Total:'}</div></div>
				<div class="price twocol last" id="total_price"><div class="gap">{displayPrice price=$total_price_without_tax}</div></div>
			</li>
			{/if}
			
			{if !($free_ship <= 0 || $isVirtualCart)}
			
			<li class="cart_free_shipping"  style="display: none;" >
				<div class="row">
					<div class="tencol" style="white-space: normal;">{l s='Remaining amount to be added to your cart in order to obtain free shipping:'}</div>
					<div id="free_shipping" class="price twocol last">{displayPrice price=$free_ship}</div>
				</div>	
	...
	{if sizeof($discounts)}
		...
		{foreach from=$discounts item=discount name=discountLoop}
			<ul class="cart_discount {if $smarty.foreach.discountLoop.last}last_item{elseif $smarty.foreach.discountLoop.first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
				<li class="cart_discount_name" colspan="2">{$discount.name}</li>
				<li class="cart_discount_description" colspan="3">{$discount.description}</li>
				<li class="cart_discount_delete"><a href="{if $opc}{$link->getPageLink('order-opc.php', true)}{else}{$link->getPageLink('order.php', true)}{/if}?deleteDiscount={$discount.id_discount}" title="{l s='Delete'}"><img src="{$img_dir}icon/delete.gif" alt="{l s='Delete'}" class="icon" width="11" height="13" /></a></td>
				<li class="cart_discount_price"><span class="price-discount">
...
<div id="cart_voucher" class="table_block">
	...
	<form action="{if $opc}{$link->getPageLink('order-opc.php', true)}{else}{$link->getPageLink('order.php', true)}{/if}" method="post" id="voucher">
		<fieldset>
			...
			<p class="submit"><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='Add'}" class="button smallBtn" /></p>
		{if $displayVouchers}
...
<p class="cart_navigation">
	{if !$opc}<a href="{$link->getPageLink('order.php', true)}?step=1{if $back}&amp;back={$back}{/if}" class="exclusive button" title="{l s='Next'}">{l s='Next'} &raquo;</a>{/if}
	<a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, $link->getPageLink('order.php'))) || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index.php')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="button_large  button" title="{l s='Continue shopping'}">&laquo; {l s='Continue shopping'}</a>
</p>
